...
class StudentsImport implements SkipsEmptyRows, SkipsOnError, SkipsOnFailure, ToModel, WithBatchInserts, WithChunkReading, WithStartRow, WithValidation
{
    ...
    public function prepareForValidation($row, $index)
    {
        ...
        if (isset($row[5]) && $row[5] !== null) {
            ...
            if ($g == 'ذكر' || $g == 'male' || $g == 'm') {
                $row[5] = 'male';
            } elseif ($g == 'أنثى' || $g == 'انثى' || $g == 'female' || $g == 'f') {
                $row[5] = 'female';
            }
        }
    ...
    public function model(array $row)
    {
        ...
        if (empty($s_national_id) || empty($g_national_id)) {
            return null;
        }
        ...
        DB::transaction(function () use ($s_first_ar, $s_last_ar, $s_first_en, $s_last_en, $s_national_id, $s_gender, $g_national_id, $g_name, $g_phone, $g_relationship) {
            ...
            if (! $guardianUser) {
                ...
                if (empty($g_name)) {
                    $g_name = 'ولي أمر '.$s_first_ar;
                }
                if (empty($g_phone)) {
                    $g_phone = $g_national_id;
                } // Fallback
