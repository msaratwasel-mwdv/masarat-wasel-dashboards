...
for (let i = 0; i < lines.length; i++) {
  ...
  if (inDict) {
    ...
    // Match ONLY top-level keys (EXACTLY 2 spaces indentation)
    // ^  [^ ] matches 2 spaces then a non-space character
    const match = line.match(/^  ([^"': ]+|"[^"]+"'|'[^']+')\s*:/);
    if (match) {
      const key = match[1].trim().replace(/^["']|["']$/g, '');
      if (keys.has(key)) {
        console.log(`Removing duplicate top-level key: ${key} at line ${i + 1}`);
        removedCount++;
        let j = i;
        if (!line.includes('},')) {
