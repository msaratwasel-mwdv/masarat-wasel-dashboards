...
try {
    ...
    $service->sendToUser(
        $parent->id,
        'student_alighted', // نوع الإشعار: نزول الطالب
        'وصل طالبك للمنزل (تجربة)',
        'لقد نزل الطالب من الحافلة الآن أمام المنزل بسلام. (رسالة اختبار)',
        [
            'student_id' => 999, 
            'bus_id' => 999, 
    ...
    echo "✅ Student Alighting Notification (SCRUM-89) sent successfully to '" . $parent->name . "' (National ID: " . $parent->national_id . ")!\n";
    echo "🔔 Check your Flutter app NOW!\n";
    echo "👀 You should see the orange 'Check-Out' (Exit) icon.\n";
} catch (\Exception $e) {
