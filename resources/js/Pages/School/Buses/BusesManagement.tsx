...
export default function BusesManagement({ auth, buses, requests, schoolLocation }: Props) {
    ...
    const handleBulkDelete = () => {
        ...
        if (confirm(t('Are you sure you want to delete the selected buses?'))) {
            router.post(route('school.buses.bulk-destroy'), {
                ids: selectedBuses,
            }, {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'active':
                return <span className="px-3 py-1.5 text-xs font-bold rounded-[10px] bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">✅ {t('Active')}</span>;
            case 'maintenance':
                return <span className="px-3 py-1.5 text-xs font-bold rounded-[10px] bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">🔧 {t('Maintenance')}</span>;
            case 'inactive':
                return <span className="px-3 py-1.5 text-xs font-bold rounded-[10px] bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400">⏸️ {t('Inactive')}</span>;
            default:
                return null;
        }
    };
        ...
        >
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-[30px] shadow-sm hover:shadow-md transition-shadow border border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-bold text-gray-500 uppercase tracking-wider">{t('Total Buses')}</p>
                                <h3 className="text-5xl font-extrabold mt-3 text-[#0e7490]">{totalBuses}</h3>
                            </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-[30px] shadow-sm hover:shadow-md transition-shadow border border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-bold text-gray-500 uppercase tracking-wider">{t('Active')}</p>
                                <h3 className="text-5xl font-extrabold mt-3 text-emerald-600">{activeBuses}</h3>
                            </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-[30px] shadow-sm hover:shadow-md transition-shadow border border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-bold text-gray-500 uppercase tracking-wider">{t('Under Maintenance')}</p>
                                <h3 className="text-5xl font-extrabold mt-3 text-orange-500">{maintenanceBuses}</h3>
                            </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-[35px] shadow-sm p-3 inline-flex gap-2">
                    ...
                    >
                        📝 {t('Requests')} {requests.length > 0 && <span className="ml-2 bg-white/20 px-2 py-0.5 rounded-full text-sm">{requests.length}</span>}
                    </button>
                </div>
                                ...
                                <div className="flex gap-4 w-full md:w-auto">
                                    {selectedBuses.length > 0 && (
                                        <button
                                            ...
                                            className="px-6 py-3 bg-red-50 text-red-600 border border-red-200 rounded-[20px] hover:bg-red-100 font-bold transition-all flex items-center gap-2"
                                        >
                                            <span>🗑️</span>
                                            {t('Delete Selected')} ({selectedBuses.length})
                                        </button>
                                    )}
                                    <button
                                        onClick={handleAddBus}
                                        className="px-8 py-3 bg-[#0e7490] text-white font-bold rounded-[20px] hover:bg-[#155e75] transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center gap-2"
                                    >
                                    ...
                                    <thead>
                                        <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
                                            <th className="px-6 py-5 w-16 text-center">
                                                <input
                                                    type="checkbox"
                                                    checked={filteredBuses.length > 0 && selectedBuses.length === filteredBuses.length}
                                                    onChange={toggleAllBuses}
                                                    className="w-5 h-5 rounded-[6px] border-gray-300 text-[#0e7490] focus:ring-[#0e7490]"
                                                />
                                            </th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase">{t('Bus Number')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase">{t('Plate Number')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase text-center">{t('Capacity')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase">{t('Driver / Supervisor')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase text-center">{t('Status')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase text-center">{t('Actions')}</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                        {filteredBuses.map(bus => (
                                            <tr key={bus.id} className="group hover:bg-cyan-50/50 dark:hover:bg-cyan-900/10 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="font-bold text-gray-900 dark:text-white text-lg">{bus.bus_number}</div>
                                                    <div className="text-xs text-gray-500">{bus.model} {bus.year}</div>
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex flex-col items-center">
                                                        <span className="font-bold text-gray-900 dark:text-white text-lg">{bus.students_count || 0} / {bus.capacity}</span>
                                                        <div className="w-20 bg-gray-200 rounded-full h-1.5 mt-1 dark:bg-gray-700">
                                                            <div 
                                                                className="bg-[#0e7490] h-1.5 rounded-full" 
                                                                style={{ width: `${Math.min(100, ((bus.students_count || 0) / bus.capacity) * 100)}%` }}
                                                            ></div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="space-y-1">
                                                        <div className="flex items-center gap-2 text-sm">
                                                            ...
                                                            <span className={bus.driver ? "text-gray-900 dark:text-white font-medium" : "text-gray-400 italic"}>
                                                                {bus.driver?.name || t('No Driver')}
                                                            </span>
                                                        </div>
                                                        <div className="flex items-center gap-2 text-sm">
                                                            <span className="text-gray-400">👀</span>
                                                            <span className={bus.supervisor ? "text-gray-900 dark:text-white font-medium" : "text-gray-400 italic"}>
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex items-center justify-center gap-2">
                                                        <button
                                                            ...
                                                            className="w-10 h-10 rounded-[12px] bg-cyan-50 text-[#0e7490] hover:bg-cyan-100 hover:scale-105 transition-all flex items-center justify-center"
                                                            title={t('Edit')}
                                                        >
                                                            ✏️
                                                        </button>
                                                        <button
                                                            ...
                                                            className="w-10 h-10 rounded-[12px] bg-red-50 text-red-600 hover:bg-red-100 hover:scale-105 transition-all flex items-center justify-center"
                                                            title={t('Delete')}
                                                        >
                                                            🗑️
                                                        </button>
                        ...
                        <div>
                            ...
                            <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/50">
                                <div className="flex items-center gap-6">
                                    <div>
                                        <h3 className="text-xl font-bold text-gray-800 dark:text-white">{t('Bus Request History')}</h3>
                                        <p className="text-sm text-gray-500">{t('Manage your transportation requests')}</p>
                                    </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase">{t('Request Type')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase text-center">{t('Buses')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase">{t('Dates')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase">{t('Reason')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase text-center">{t('Status')}</th>
                                            <th className="px-6 py-5 text-sm font-bold text-[#0e7490] dark:text-cyan-400 uppercase text-center">{t('Actions')}</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                        {filteredRequests.map(request => (
                                            <tr key={request.id} className="group hover:bg-cyan-50/50 dark:hover:bg-cyan-900/10 transition-colors">
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4">
                                                    <p className="text-sm text-gray-600 dark:text-gray-300 line-clamp-1 max-w-xs" title={request.reason}>
                                                        {request.reason}
                                                    </p>
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`px-3 py-1.5 rounded-[10px] text-xs font-bold capitalize inline-flex items-center gap-1 ${
                                        ...
                                        {filteredRequests.length === 0 && (
                                            <tr>
                                                <td colSpan={6} className="py-16 text-center">
                                                    ...
                                                    <p className="text-gray-500">{t(requestSearchQuery ? 'No requests match your search' : 'Click New Request to submit one')}</p>
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
