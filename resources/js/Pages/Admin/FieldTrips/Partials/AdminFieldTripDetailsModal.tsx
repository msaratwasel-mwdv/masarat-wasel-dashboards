...
export default function AdminFieldTripDetailsModal({ show, onClose, tripId }: Props) {
    ...
    const fetchTripDetails = async () => {
        setLoading(true);
        try {
            // Using the admin route for fetching details
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    ...
                                    {tripData?.school && (
                                        <span className="px-2 py-1 bg-brand-yellow text-brand-dark rounded-lg text-[8px] font-black uppercase tracking-widest leading-none">
                                            {tripData.school.name}
                                        </span>
                                    )}
                                </div>
                                <p className="text-white/50 text-[10px] font-bold uppercase tracking-widest mt-2 lowercase italic">
                                    ID: #{tripId} • {tripData?.status === 'planned' ? t('Pending Review') : t('In Progress')}
                                </p>
                            </div>
                        </div>
                ...
                <div className="p-8 overflow-y-auto flex-1 custom-scrollbar">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center h-64 space-y-4">
                            ...
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('Processing...')}</p>
                        </div>
                    ) : tripData ? (
                        <div className="space-y-8 animate-fadeIn">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div className="p-6 bg-gray-50 dark:bg-gray-900/50 rounded-3xl border border-gray-100 dark:border-gray-800">
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        <span className="px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-600 rounded text-[8px] font-black uppercase tracking-widest">Validated</span>
                                    </div>
                                </div>
                                <div className="p-6 bg-brand-navy text-white rounded-3xl shadow-xl shadow-brand-navy/10 flex flex-col justify-center">
                                    <p className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-1">{t('Quote')}</p>
                                    <p className="text-2xl font-black">{tripData.cost ? `${tripData.cost} ر.س` : t('Pending Approval')}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-widest ml-1">{t('Geographic Context')}</h3>
                                    <FieldTripMapDisplay
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-widest ml-1">{t('Field Faculty')}</h3>
                                    <div className="space-y-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                                            ...
                                            <div key={teacher.id} className="p-4 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl flex items-center justify-between shadow-sm">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{teacher.phone || '---'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        {tripData.external_members?.map((m: any, idx: number) => (
                                            <div key={idx} className="p-4 bg-purple-50/30 dark:bg-purple-900/10 border border-purple-100/50 dark:border-purple-800/30 rounded-2xl flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 bg-purple-500 rounded-lg flex items-center justify-center text-white text-[10px] font-black uppercase">E</div>
                                                    <div>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center px-1">
                                    <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-widest">{t('Deployment List')}</h3>
                                    <span className="px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-[9px] font-black uppercase tracking-widest text-gray-500">
                                        {tripData.students?.length} {t('Students')}
                                    </span>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 overflow-y-auto max-h-96 pr-2 custom-scrollbar">
                                    {tripData.students?.map((s: any) => (
                                        <div key={s.id} className="p-4 bg-gray-50/50 dark:bg-gray-900/30 border border-gray-100 dark:border-gray-800 rounded-2xl flex items-center gap-3 transition-all hover:bg-white dark:hover:bg-gray-800 hover:shadow-md cursor-default group">
                                            ...
                                            <div>
                                                <p className="font-black text-xs text-gray-800 dark:text-gray-100 group-hover:text-brand-navy transition-colors">{s.first_name_ar} {s.last_name_ar}</p>
                                ...
                                <div className="p-8 bg-gradient-to-br from-brand-navy to-brand-dark rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                                        <div className="text-center md:text-right">
                                            <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] mb-2">{t('Logistic Asset')}</p>
                                        ...
                                        <div className="flex gap-12 text-center">
                                            ...
                                            <div>
                                                <p className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-1">{t('Assistant')}</p>
                        ...
                        <div className="flex flex-col items-center justify-center h-64 text-red-400 text-center">
                             ...
                             <p className="font-black text-[10px] uppercase tracking-widest">{t('Asset synchronization failed')}</p>
                        </div>
                    )}
                </div>
