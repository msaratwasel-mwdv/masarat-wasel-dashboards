...
export default function DriversIndex({ drivers, counts, filters }: Props) {
  ...
  const [previewLicenseFront, setPreviewLicenseFront] = useState<string | null>(
    null
  );
  const [previewLicenseBack, setPreviewLicenseBack] = useState<string | null>(
    null
  );
  ...
  const openEditModal = (driver: Driver) => {
    ...
    setPreviewLicenseFront(
      driver.license_front_image
        ? `/storage/${driver.license_front_image}`
        : null
    );
    setPreviewLicenseBack(
      driver.license_back_image ? `/storage/${driver.license_back_image}` : null
    );
    setData({
      _method: "put",
            ...
            {showDetailsModal && selectedDriver && (
              ...
              >
                <div
                  ...
                  } rounded-3xl overflow-hidden shadow-2xl p-0`}
                >
                  {/* Backdrop Header */}
                  <div className="h-32 bg-gradient-to-r from-brand-navy to-brand-dark flex items-end px-8 pb-4 relative">
                    ...
                    <div className="absolute -bottom-12 left-8 w-24 h-24 rounded-2xl border-4 border-white dark:border-gray-900 bg-gray-100 overflow-hidden shadow-lg">
                      {selectedDriver.image ? (
                        <img
                          src={`/storage/${selectedDriver.image}`}
                          className="w-full h-full object-cover"
                        />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-3xl font-bold text-brand-dark bg-yellow-50">
                  ...
                  <div className="pt-16 px-8 pb-8">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-brand-navy font-bold">
                          {selectedDriver.name_en || ""}
                        </p>
                        <div className="flex gap-2 mt-2">
                          <StatusBadge
                            status={
                              selectedDriver.driver?.status === "active"
                                ? "active"
                                : "inactive"
                            }
                          />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
                      ...
                      <div className="space-y-4">
                        ...
                        <InfoRow
                          ...
                          value={selectedDriver.national_id}
                          isDark={isDark}
                        />
                        <InfoRow
                          icon={<Phone size={16} />}
                      ...
                      <div className="space-y-4">
                        ...
                        <InfoRow
                          icon={<CreditCard size={16} />}
                          label={isRTL ? "رقم الرخصة" : "License Number"}
                          value={selectedDriver.driver?.license_number || "—"}
                          isDark={isDark}
                        />
                        <InfoRow
                          icon={<Calendar size={16} />}
                          label={isRTL ? "انتهاء الرخصة" : "License Expiry"}
                          value={
                            selectedDriver.driver?.license_expiry_date || "—"
                          }
                          isDark={isDark}
                          highlight={IS_EXPIRED(
                            selectedDriver.driver?.license_expiry_date
                          )}
                        />
                        <InfoRow
                          icon={<BusIcon size={16} />}
                          label={isRTL ? "الحافلة المعينة" : "Assigned Bus"}
                          value={
                            selectedDriver.assigned_bus?.bus_number ||
                    ...
                    <div className="mt-12">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        <MediaCard
                          label={isRTL ? "صورة السائق" : "Driver Photo"}
                          src={selectedDriver.image}
                          isDark={isDark}
                          isRTL={isRTL}
                        />
                        <MediaCard
                          label={isRTL ? "الرخصة (أمام)" : "License (Front)"}
                          src={
                            selectedDriver.license_front_image ||
                            selectedDriver.driver?.license_front_image
                          }
                          isDark={isDark}
                          isRTL={isRTL}
                        />
                        <MediaCard
                          label={isRTL ? "الرخصة (خلف)" : "License (Back)"}
                          src={
                            selectedDriver.license_back_image ||
                            selectedDriver.driver?.license_back_image
                          }
                          isDark={isDark}
                          isRTL={isRTL}
                        />
            ...
            >
              ...
              >
                ...
                <div className="flex justify-between px-4 mt-2 text-xs font-bold text-gray-500">
                  ...
                  <span>
                    {isRTL ? "بيانات الاتصال والرخصة" : "Contact & License"}
                  </span>
                </div>
              </div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <label className="cursor-pointer text-xs font-bold text-brand-navy underline">
                              {isRTL ? "اختيار" : "Choose"}
                              <input
                                type="file"
                                className="hidden"
                  ...
                  >
                    {currentStep === 1 ? (
                      <button
                        ...
                        className={`px-6 py-2.5 rounded-xl font-bold text-sm shadow-sm transition-opacity bg-brand-navy text-white hover:opacity-90`}
                      >
                        {isRTL ? "التالي" : "Next"}
                      </button>
                    ) : (
                      <button
                        type="submit"
                        disabled={processing}
                        className={`px-6 py-2.5 rounded-xl font-bold text-sm shadow-sm transition-opacity disabled:opacity-50 bg-brand-yellow text-brand-dark hover:opacity-90`}
                      >
                        {isEditing
                          ? isRTL
                            ? "حفظ التعديلات"
      ...
      <div className="flex items-center gap-3">
        <div
          className={`p-1.5 rounded-lg ${
            isDark ? "bg-gray-800 text-gray-500" : "bg-gray-50 text-brand-navy"
          }`}
        >
          {icon}
        </div>
    ...
    >
      ...
      <div
        className={`aspect-[4/3] rounded-2xl overflow-hidden border-2 ${
          isDark ? "bg-gray-800 border-gray-700" : "bg-gray-100 border-gray-50"
        }`}
      >
        {src ? (
          <a
...
function PersonStatCard({
  ...
  color: keyof typeof personStatColorMap;
  isDark: boolean;
  isRTL: boolean;
}) {
