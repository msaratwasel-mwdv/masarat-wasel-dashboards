<?php
$schoolId = 3; // The user's active school
$bus = \App\Models\Bus::where('school_id', $schoolId)->first();
if(!$bus) { echo "No bus for school 3\n"; exit; }
$school = \App\Models\School::find($schoolId);
$students = \App\Models\Student::where('forth_bus_id', $bus->id)->orWhere('back_bus_id', $bus->id)->take(3)->get();
if($students->isEmpty()) {
    $students = \App\Models\Student::where('school_id', $schoolId)->take(3)->get();
}
...
foreach ($dates as $day) {
    ...
    \App\Models\Trip::where('school_id', $school->id)
        ->where('bus_id', $bus->id)
        ->whereDate('trip_date', $day->toDateString())
        ->delete();
...
echo "Dummy trips created successfully for school ID 3!\n";
